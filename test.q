...
calc_vwap: { [cur_st;days]
    ...
    trades: update vol_price: price*volume from trades;
    `trades set trades;
    /`t set  ((cols trades), `CNT) xcols grid ,' (update CNT:i from trades) asof grid;
    t: grid ,' (update CNT:i from trades) asof grid;
    /trade_bar:update volbar,vol_price: sum each t[`CNT] _ trades[`volume], CNT: deltas CNT from t;
    `trade_bar set update volbar: sum each t[`CNT] _ trades[`volume], vol_price: sum each t[`CNT] _ trades[`vol_price], CNT: deltas CNT from t;
